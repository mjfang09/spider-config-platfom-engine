...
@File    :   dead_data_deduplicate_msg_handle_threads.py
@Time    :   2024/5/7 10:38
...
from utils.spider_constant import FILTER_DEAD_MSG_CONFIG_FILE_PATH, DEAD_DATA_DEDUPLICATE_FLAG, \
    DEAD_LC_JZ_DATA_TO_BE_DEDUPLICATED_QUEUE_NAME, \
    DEAD_TEST_LC_JZ_DATA_TO_BE_DEDUPLICATED_QUEUE_NAME

LOG_APP_ID = ''
LOGGER_NAME = 'spider.handle_msg_from_data_deduplicate_dead_queue'
    ...
    try:
        ...
        if not (sourceid and queue_name):
            ...
            LOGGER.other_op.info(
                f'''配置表sourceid:{sourceid} - {DEAD_DATA_DEDUPLICATE_FLAG} - 网址：{body_dict.get("_url", "")} - {body_dict} - 报错：获取到的当前消息中的sourceid或send_to_next_mq参数为空！''')
        ...
        if db_env == '185':
            config_key = DEAD_TEST_LC_JZ_DATA_TO_BE_DEDUPLICATED_QUEUE_NAME
        elif db_env == 'formal':
            config_key = DEAD_LC_JZ_DATA_TO_BE_DEDUPLICATED_QUEUE_NAME
        else:
        ...
        if sourceid not in source_id_list:
            ...
            if ttl_discard_flag:
                LOGGER.other_op.info(
                    f'''配置表sourceid:{sourceid} - {DEAD_DATA_DEDUPLICATE_FLAG} - 网址：{body_dict.get("_url", "")} - {body_dict} - 当前消息过期了,丢弃！''')
                ch.basic_ack(delivery_tag=method.delivery_tag)
                LOGGER.receive_message.info(f"00 - [tag] - {method.delivery_tag} -消息已确认消费")
                return
            else:
                ...
                return
        try:
            body_dict['program_flag'] = DEAD_DATA_DEDUPLICATE_FLAG
            ...
            LOGGER.other_op.info(
                f'''配置表sourceid:{sourceid} - {DEAD_DATA_DEDUPLICATE_FLAG} - 网址：{body_dict.get("_url", "")} - {body_dict} - 消息推送至{queue_name}成功！''')
        except Exception as e:
            LOGGER.other_op.info(
                f'''配置表sourceid:{sourceid} - {DEAD_DATA_DEDUPLICATE_FLAG} - 网址：{body_dict.get("_url", "")} - {body_dict} - 消息推送至{queue_name}失败！- {str(traceback.format_exc())}''')
            raise Exception(
                f'''配置表sourceid:{sourceid} - {DEAD_DATA_DEDUPLICATE_FLAG} - 网址：{body_dict.get("_url", "")} - {body_dict} - 消息推送至{queue_name}失败！- {str(traceback.format_exc())}''')
    except Exception as e:
...
def dlx_msg_handler():
    ...
    LOGGER.other_op.info("00 - 死信队列 - 数据去重 - 消息处理程序启动!")

    if db_env == '185':
        queue_name = DEAD_TEST_LC_JZ_DATA_TO_BE_DEDUPLICATED_QUEUE_NAME
    elif db_env == 'formal':
        queue_name = DEAD_LC_JZ_DATA_TO_BE_DEDUPLICATED_QUEUE_NAME
    else:
        raise Exception(f'未知的运行环境：{db_env}')
